...
class Wmenu{
	...
	public function eventList($data)
	{
		...
		$sql = "SELECT A.*,B.name AS mname,C.bak AS qrcode FROM same_wmenu_event A left join same_wmenu B ON B.id=A.mid left join same_qrcode C ON A.qid = C.id WHERE $where ORDER BY A.id DESC  limit $offset,$rows";
		$command = $this->_db->createCommand($sql);		
		$menuAll = $command->select()->queryAll();
		$menuAll = array("total"=>$count,"rows"=>$menuAll);
		return json_encode($menuAll);
	...
	public function eventUpdate($data)
	{
		...
		if ($event == 'qrcode') {
			$sqlCheck = "SELECT id FROM same_wmenu_event WHERE qid=:qid AND event=:event AND id<>".$id;
			$command = $this->_db->createCommand($sqlCheck);
		...
		try{
			$sql = "UPDATE same_wmenu_event SET mid=:mid, qid=:qid, event=:event, msgtype=:msgtype,content=:content, title=:title, description=:description, picUrl=:picUrl, url=:url, keyword=:keyword WHERE id=:id";
			$command = $this->_db->createCommand($sql);
			$command->bindParam(':id',$id,PDO::PARAM_INT);
			$command->bindParam(':mid',$mid,PDO::PARAM_STR);
			$command->bindParam(':qid',$qid,PDO::PARAM_STR);
	...
	public function qrcodeList($data)
	{
		...
		$sql = "SELECT same_qrcode.*,a.num,b.num2 from same_qrcode left join (select count(*) as num,ticket from scenelog group by ticket) as a on same_qrcode.ticket = a.ticket left join (select count(*) as num2,ticket from scenelog where type = 1 group by ticket) as b on same_qrcode.ticket = b.ticket ORDER BY id DESC  limit $offset,$rows";
		$command = $this->_db->createCommand($sql);		
		$menuAll = $command->select()->queryAll();
		$menuAll = array("total"=>$count,"rows"=>$menuAll);
		return json_encode($menuAll);
