...
class Weixin{
	...
	private $_TOKEN = 'guerlain';
	private $_appid = 'wx5222b62120b15c66';
	private $_secret = '07e79ddc7126a5f2880f100dfb791273';
    ...
    public function responseMsg($postStr)
    {
		if (!empty($postStr)){
				...
				if($msgType=='text'){
					...
					echo $info_sql = "select * from `same_store` where lat<>0 and (lat between {$latsmall} and {$latbig}) and (lng between {$lngsmall} and {$lngbig})";
					exit;$rs = Yii::app()->db->createCommand($info_sql)->queryAll();
					if(!$rs){
